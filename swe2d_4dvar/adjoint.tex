...
\label{eq:eq8}
\end{equation}
这里选择负号，是为了让本文的伴随变量与代码中的变量
\texttt{lam\_curr} 采用同一符号约定：观测残差的梯度以正号注入伴随，
最终梯度为背景项梯度加上初值伴随。
...
\medskip
\noindent\textbf{Step 2: 约束项的变分。}利用 $\delta\mathbf{f}=A\,\delta\mathbf{q}$ 与 $\delta\mathbf{g}=B\,\delta\mathbf{q}$，先写出约束积分本身的一阶变分：
\[
...
\medskip
\noindent\textbf{Step 4: 代回最优性条件 $\delta\mathcal{L}=0$。}由于 \eqref{eq:eq8} 中约束项前面取负号，代回时需要把 Step 3 的分部积分结果整体取负。暂先假设空间边界项
\[
...
\label{eq:eq25}
\end{equation}
该近似等价于假定 Riemann 问题的耗散结构在邻域内不随状态变化，在非线性不强的浅水波问题中通常足够准确。不过当前 \texttt{swe2d\_4dvar.c} 并未采用冻结耗散近似，而是采用\textbf{分段导数}：\texttt{rusanov\_x\_jacobians()} 与 \texttt{rusanov\_y\_jacobians()} 先计算两侧速度 $a_L,a_R$，再只对达到最大值的一侧保留 $\nabla s$。因此代码实现对应的是 \eqref{eq:eq22}--\eqref{eq:eq23} 的分支形式；只有在显式丢弃外积项时才退化为 \eqref{eq:eq25}。
...
在 \texttt{euler\_adjoint()} 中，这四个界面 Jacobian 的转置乘法通过辅助函数 \texttt{add\_matT\_vec\_to\_cell()} 实现。给定一个 $3\times3$ Jacobian 矩阵 $J$ 和一个三维向量 $\mathbf{v}$，该函数计算
\[
...
= {}& \bm{\lambda}^+_{i,j}
- \frac{\Delta t}{\Delta x}
\bigl(\widehat{F}_{R,(i,j)}^{\mathsf T}
+\widehat{F}_{L,(i,j)}^{\mathsf T}\bigr)\bm{\lambda}^+_{i,j} \\
&+ \frac{\Delta t}{\Delta x}\widehat{F}_{R,(i-1,j)}^{\mathsf T}
\bm{\lambda}^+_{i-1,j}
- \frac{\Delta t}{\Delta y}
\bigl(\widehat{G}_{T,(i,j)}^{\mathsf T}
+\widehat{G}_{B,(i,j)}^{\mathsf T}\bigr)\bm{\lambda}^+_{i,j} \\
&+ \frac{\Delta t}{\Delta y}\widehat{G}_{T,(i,j-1)}^{\mathsf T}
\bm{\lambda}^+_{i,j-1},
\end{align*}
...
\texttt{../swe2d\_4dvar\_package/swe2d\_4dvar\_report.tex} 对一个相近的 4D-Var 演示程序给出了清晰的软件说明，但那份程序采用低维控制向量和有限差分梯度；当前目录中的 \texttt{swe2d\_4dvar.c} 已经转向全初值控制和离散伴随梯度。因此本文只借鉴其"模型--同化--输出--验证"的组织方式，而不沿用其中的有限差分梯度公式作为当前实现说明。

\subsection{核心函数与数学对象的对应}

当前程序全部位于单个 C 文件中。与本文推导对应的关键函数如下：
\begin{itemize}
\item \texttt{flux\_x()}, \texttt{flux\_y()}：计算物理通量 $\mathbf{f}$ 和 $\mathbf{g}$；
\item \texttt{rusanov\_x()}, \texttt{rusanov\_y()}：计算 Rusanov 数值通量；
\item \texttt{rusanov\_x\_jacobians()}, \texttt{rusanov\_y\_jacobians()}：计算界面通量对左右状态的 Jacobian，即 \eqref{eq:eq22}--\eqref{eq:eq23} 及其 $y$ 方向对应式；
\item \texttt{euler\_update()}：实现 \eqref{eq:eq14} 的前向 Euler 子步；
\item \texttt{step\_rk2\_store()}：执行两次 Euler 子步并保存 $\mathbf{Q}^{(1)}$，对应 \eqref{eq:eq15}；
\item \texttt{euler\_adjoint()}：实现 Euler 子步 Jacobian 的转置累加，对应 \eqref{eq:eq29}；
\item \texttt{rk2\_adjoint\_step()}：按 \eqref{eq:eq31}--\eqref{eq:eq33} 反传 RK2；
\item \texttt{obs\_gradient\_add()}：把 $H^\mathsf{T}R^{-1}(H\mathbf{Q}-\mathbf{y})$ 注入伴随变量；
\item \texttt{evaluate\_cost\_gradient()}：一次完成前向轨道保存、代价函数计算、反向伴随积分和初值梯度组装；
\item \texttt{line\_search\_two\_way()}：使用 Armijo 条件和曲率条件选择步长。
\end{itemize}

\subsection{默认参数}

当前默认配置见表 \ref{tab:current-params}。这些参数对应 \texttt{swe2d\_4dvar.c} 中的宏定义，而不是相邻 package 报告中的 $36\times36$、低维控制配置。

\begin{table}[htbp]
\centering
\caption{当前 \texttt{swe2d\_4dvar.c} 的默认配置}
\label{tab:current-params}
\begin{tabular}{lcc}
\toprule
参数 & 记号 & 默认值 \\
\midrule
网格规模 & $N_x\times N_y$ & $60\times60$ \\
空间区域 & $L_x\times L_y$ & $1\times1$ \\
观测点数 & $N_{\mathrm{obs}}$ & $16$ \\
观测周期数 & $N_{\mathrm{cycles}}$ & $30$ \\
单周期窗口长度 & $\Delta T$ & $0.04$ \\
每周期 RK2 步数 & $N_{\mathrm{step}}$ & $20$ \\
固定时间步长 & $\Delta t$ & $0.002$ \\
观测误差标准差 & $\sigma_o$ & $0.01$ \\
背景水深标准差 & $\sigma_h$ & $0.02$ \\
背景动量标准差 & $\sigma_m$ & $0.005$ \\
最大优化迭代数 & $N_{\max}$ & $12$ \\
\bottomrule
\end{tabular}
\end{table}
...
程序运行后写入 \texttt{var\_output/}。其中最重要的诊断文件包括：
\begin{itemize}
\item \texttt{cost\_history.csv}：每次优化迭代的总代价、梯度范数和接受步长；
\item \texttt{iter\_cycle\_misfit.csv}：每次迭代、每个观测周期的观测项代价和观测 RMS；
\item \texttt{trajectory\_metrics.csv}：背景轨道和分析轨道相对于真值的水深 RMSE；
\item \texttt{truth\_cycle*.csv}, \texttt{background\_cycle*.csv}, \texttt{analysis\_cycle*.csv}：各输出时刻的格点场；
\item \texttt{obs\_config.csv}：观测点所在网格单元；
\item \texttt{fd\_adjoint\_check.csv}：随机方向上的 Taylor 梯度检验；
\item \texttt{fd\_adjoint\_components.csv}：抽样控制分量上的中心有限差分与伴随梯度对比；
\item \texttt{fd\_adjoint\_components\_summary.txt}：逐分量检验的范数、相关系数和最大误差摘要。
\end{itemize}

\subsection{优化算法}

当前优化器不是相邻 package 报告中的有限差分最速下降。它先利用离散伴随得到全状态梯度，再用对角背景方差作为预条件，令
\[
\mathbf{p}
= -\operatorname{diag}(\sigma_h^2,\sigma_m^2,\sigma_m^2)\,\nabla J.
\]
之后执行带有 Armijo 充分下降和弱 Wolfe 型曲率检查的线搜索：
\[
J(\mathbf{Q}^0+\alpha\mathbf{p})
\le J(\mathbf{Q}^0) + c_1\alpha\,\nabla J^\mathsf{T}\mathbf{p},
\qquad
\nabla J(\mathbf{Q}^0+\alpha\mathbf{p})^\mathsf{T}\mathbf{p}
\ge c_2\,\nabla J^\mathsf{T}\mathbf{p}.
\]
其中 $c_1=10^{-4}$，$c_2=0.9$。这比单纯回溯更稳健，因为它不仅要求目标函数下降，也避免接受过短且曲率信息不合理的步长。
...
当前代码已经在主程序启动后执行 Taylor 检验。一次典型运行的输出为
\begin{center}
\small
$
\begin{array}{c|ccccc}
...
\hline
\mathrm{Taylor\ residual}
& 1.3782\times10^2 & 1.3782 & 1.3782\times10^{-2}
& 1.3782\times10^{-4} & 1.3782\times10^{-6}
\end{array}
$
\end{center}
其中 $\mathrm{Taylor\ residual}$ 表示
\[
\left|J(\mathbf{Q}^0+\varepsilon\delta\mathbf{Q}^0)-J(\mathbf{Q}^0)
-\varepsilon\langle\nabla J,\delta\mathbf{Q}^0\rangle\right|.
\]
误差随 $\varepsilon$ 每缩小 $10$ 倍而约缩小 $100$ 倍，呈现二阶 Taylor 余项行为。这是比"优化过程能下降"更直接的离散伴随验证，因为它检验的是梯度方向导数本身。

图 \ref{fig:fd-adjoint-check} 将同一检验可视化。左图画的是上式定义的
$1-|\Phi(\varepsilon)|$，因此理想极限是 $0$；右图画的是 Taylor 余项。当前结果显示，较大的扰动会受到非线性项影响，$1-|\Phi|$ 不必接近零；当 $\varepsilon$ 减小到 $10^{-5}$ 时，$\Phi$ 已接近 $1$，$1-|\Phi|$ 约为 $6.7\times10^{-3}$。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{var_output/fd_adjoint_check.png}
    \caption{有限差分方向导数与离散伴随方向导数的梯度检验。左图为 $1-|\Phi(\varepsilon)|$，右图为 Taylor 余项。}
    \label{fig:fd-adjoint-check}
\end{figure}

随机方向的 Taylor 检验可以验证整体方向导数，但不容易定位某个变量分量或索引映射错误。因此当前程序还会从完整初值控制向量
$\mathbf{Q}^0\in\mathbb{R}^{3N_xN_y}$ 中确定性抽取若干分量，并对每个分量做中心有限差分：
\begin{equation}
\left(\nabla J\right)_p^{\mathrm{FD}}
=\frac{J(\mathbf{Q}^0+\varepsilon\mathbf{e}_p)-
       J(\mathbf{Q}^0-\varepsilon\mathbf{e}_p)}{2\varepsilon},
\qquad \varepsilon=10^{-6}.
\label{eq:eq40}
\end{equation}
这里采用中心差分而不是一阶向前差分，是为了消除背景二次项带来的
$\mathcal{O}(\varepsilon/\sigma^2)$ 截断偏差。对于动量分量，
$\sigma_m=0.005$，这一偏差在向前差分中会比较明显。

默认运行抽取 $24$ 个分量，覆盖 $h,hu,hv$ 三类变量。当前结果为
\[
\frac{\|\nabla J_{\mathrm{adj}}-\nabla J_{\mathrm{FD}}\|_2}
     {\|\nabla J_{\mathrm{FD}}\|_2}
=1.64\times 10^{-7},
\qquad
\mathrm{corr}(\nabla J_{\mathrm{adj}},\nabla J_{\mathrm{FD}})=1.000000000000.
\]
最大绝对差异为 $1.03\times 10^{-4}$，出现在一个 $h$ 分量上。
图~\ref{fig:fd-adjoint-components} 给出了伴随梯度与中心有限差分梯度的散点、逐分量叠加以及误差图。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{var_output/fd_adjoint_components.png}
    \caption{抽样控制分量上的中心有限差分与离散伴随梯度对比。该图比单一方向 Taylor 检验更容易暴露变量分量或索引映射错误。}
...
当前代码已经完成了基本离散伴随实现、Taylor 检验和抽样逐分量检验。
如果下一步要继续提高这份 VarDA 原型，一个务实的实现顺序是：
\begin{enumerate}
\item 将 Taylor 检验和抽样逐分量检验固定为回归测试，覆盖不同网格、观测网络和初值扰动；
\item 将优化器从预条件最速下降升级为 L-BFGS 或共轭梯度；
\item 为长窗口或高分辨率网格加入检查点重计算，减少保存完整轨迹的内存代价；
\item 在当前对角背景协方差之外，引入空间相关背景误差模型；
\item 最后再考虑更多观测类型、质量控制和混合集合--变分扩展。
\end{enumerate}
...
对当前这份 4D-Var 原型，以上数值结果至少说明了四件事。第一，离散伴随梯度通过 Taylor 检验和抽样逐分量中心差分检验，说明反向传播与前向离散在数值上是一致的。第二，离散伴随驱动的梯度下降在数值上是工作的，因为总代价函数能够稳定下降。第三，优化后的轨道相对于背景轨道有显著改进，说明反传得到的初值梯度方向具有正确的物理信息。第四，若考察的是逐周期 misfit，就必须区分"全局目标单调下降"和"局部观测项逐次下降"这两个不同概念；对单窗口 4D-Var 来说，后者本来就不应被当作必需性质。
